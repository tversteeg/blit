...
impl Animation {
    ...
    pub fn start_from_tag(buffer: &AnimationBlitBuffer, tag: String, repeat: bool) -> Result<Self, AnimationError> {
        ...
        for frame in frame_tags {
            if tag == frame.name {
                return Ok(Animation::start(frame.from as usize, frame.to as usize, repeat));
            }
        }
